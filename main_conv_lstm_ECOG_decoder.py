...
def train():
  ...
  with tf.Graph().as_default():
    ...
    with tf.variable_scope('conv_lstm', initializer = tf.random_uniform_initializer(-.01, 0.1)):
      cell_encode = BasicConvLSTMCell.BasicConvLSTMCell([10,9], [3,3], 256,'lstm_1')
      #cell_decode = BasicConvLSTMCell.BasicConvLSTMCell([10,9], [3,3], 128)
      new_state_encode = cell_encode.zero_state(FLAGS.batch_size, tf.float32) 
    with tf.variable_scope('conv_lstm', initializer = tf.random_uniform_initializer(-.01, 0.1)):
      cell_predict = BasicConvLSTMCell.BasicConvLSTMCell([10,9], [3,3], 256,'lstm_2')
      #new_state_decode = cell_decode.zero_state(FLAGS.batch_size, tf.float32)

...
tf.app.flags.DEFINE_string('train_dir', './checkpoints/train_store_conv_lstm_ECOG_origin',
...
def train():
  ...
  data_handler = VideoPatchDataHandler(FLAGS.seq_length,FLAGS.batch_size,5,'valid')
  with tf.Graph().as_default():
    ...
    with tf.variable_scope('conv_lstm_1', initializer = tf.random_uniform_initializer(-.01, 0.1)):
      cell = BasicConvLSTMCell.BasicConvLSTMCell([10,9], [3,3], 128,'conv_lstm_1')
    ...
    with tf.variable_scope('conv_lstm_2', initializer = tf.random_uniform_initializer(-.01, 0.1)):
      cell_1 = BasicConvLSTMCell.BasicConvLSTMCell([10,9], [3,3], 128,'conv_lstm_2')
      new_state_1 = cell_1.zero_state(FLAGS.batch_size, tf.float32)
    # conv network
    for i in xrange(FLAGS.seq_length-1):
      # conv1
      #if i < FLAGS.seq_start:
      #  conv1 = ld.conv_layer(x_dropout[:,i,:,:,:], 3, 1, 64, "encode_1")
      #else:
      ...
      y_2, new_state_1 = cell_1(y_1,new_state_1)
      # conv5
    ...
    new_state_gen_1 = cell_1.zero_state(FLAGS.batch_size, tf.float32)
    for i in xrange(20):
      # conv1
      if i < FLAGS.seq_start:
        conv1 = ld.conv_layer(x[:,i,:,:,:], 3, 1, 64, "encode_1")
      ...
      y_2, new_state_gen_1 = cell_1(y_1, new_state_gen_1)
      # conv5
      conv5 = ld.transpose_conv_layer(y_2, 1, 1, 64, "decode_5")
      # conv6
      conv6 = ld.transpose_conv_layer(conv5, 3, 1, 64, "decode_6")
      # conv7
      conv7 = ld.transpose_conv_layer(conv6, 3, 2, 64, "decode_7")
